#!/usr/bin/env python3
"""Find the army (not army-group) create poster."""
...
def xrefs_fn(data, sections, fn_rva):
    ...
    while True:
        ...
        hits.append((hex(vr) if vr else vr, name))
        p = i + 8
        if len(hits) >= 12:
            break
    return hits
...
def main():
    ...
    print("== RTTI Army/Front/Group commands ==")
    start = 0
    while True:
    ...
    for rva, label in (
        (0x01828E80, "NewFront helper 0x01828E80"),
        (0x01A23290, "NewFront poster 0x01A23290"),
        (0x00F41450, "Idler NewFront helper"),
        (0x0182DBB0, "COrderNewFrontCommand[10]"),
        (0x00EEE3A0, "front factory"),
    ):
    ...
    print("\nLEA COrderNewFrontCommand")
    for lea in lea_to(data, sections, 0x029E4180, 16):
        pfn = fn_for(funcs, lea)
        print(f"  0x{lea:08x} {None if not pfn else (hex(pfn[0]), hex(pfn[1]))} {hexdump(data, sections, max(0, lea-8), 24)}")
